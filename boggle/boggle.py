...
def find(board, word):
    ...
    for i in range(0, 5):
        for j in range(0, 5):
            if helper(board, word, i, j, seen=set()):
                return True

    return False

def helper(board, word, i, j, seen):
    """Can we find a word on board, starting at x, y?"""

    if board[i][j] != word[0]:
        print("%-6s%d,%d  %-3s%-8s%-30s" % ("NO",i,i, board[i][j], word,
                                            seen))
        return False

    if (i, j) in seen:
        print("%-6s%d,%d  %-3s%-8s%-30s" % ("SEEN", i, j, board[i][j], word,
                                            seen))
        return False

 
    if len(word) == 1:
        print("%-6s%d,%d  %-3s%-8s%-30s" % ("WIN", i, j, board[i][j], word,
                                            seen))
        return True


    print("%-6s%d,%d  %-3s%-8s%-30s" % ("OK", i, j, board[i][j], word, seen))


    seen = seen | {(i, j)}

    if i > 0:
        if helper(board, word[1:], i - 1, j, seen):
            return True
    ...
    if j > 0:
        if helper(board, word[1:], i, j - 1, seen):
            return True
